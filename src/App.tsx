...
async function testConnection() {
  try {
    ...
    if (error.message && error.message.includes("the client is offline")) {
      console.error(
        "Firestore Error: Failed to get document because the client is offline. Please check your Firebase configuration.",
      );
    }
  }
}
    ...
    <div className="max-w-4xl mx-auto text-center">
      ...
      >
        ...
        <p className="text-lg text-white/60 mb-10 max-w-2xl mx-auto">
          Insight AI uses advanced AI to reverse-engineer viral videos. Identify
          outliers, leverage high-performing tags, and optimize your content for
          10x growth.
        </p>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-white/60 mb-8">
            Stop overpaying for your toolkit. Unlock premium features at a
          ...
          {PLANS.map((plan) => {
            ...
            return (
              ...
              >
                ...
                <button
                  onClick={() =>
                    onSelect({ ...plan, billingPeriod: billing, price })
                  }
                  className={cn(
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        <h2 className="text-4xl font-bold mb-4">
          What will you make videos about?
        </h2>
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6">
            ...
            >
              {loading ? (
                ...
                <>
                  Pay ${isAnnual ? plan.annualTotal : plan.price * 12} & Start
                  Growing
                </>
              )}
            </button>
          ...
          <div className="border-t border-white/10 pt-4 mb-8">
            ...
            {isAnnual && (
              <p className="text-xs text-orange-400 mt-1">
                You save ${(plan.monthlyPrice - plan.annualPrice) * 12}/year vs
                monthly
              </p>
            )}
          </div>
...
const Dashboard = ({ user }: { user: FirebaseUser }) => {
  ...
  const comparisonData = React.useMemo(() => {
    ...
    return [
      {
        name: "Views",
        target: Number(target.viewCount),
        competitor: comp ? Number(comp.viewCount) : 0,
      },
      {
        name: "Likes",
        target: Number(target.likeCount),
        competitor: comp ? Number(comp.likeCount) : 0,
      },
      {
        name: "Comments",
        target: Number(target.commentCount),
        competitor: comp ? Number(comp.commentCount) : 0,
      },
    ];
  }, [analysisResult]);
  ...
  const [studioTool, setStudioTool] = useState<
    "thumbnail" | "keywords" | "ideas" | "script" | "clips" | "paraphrase"
  >("thumbnail");
  ...
  const handleStudioAction = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        await saveAnalysisToHistory("video", `studio:${studioTool}`, {
          input: studioInput,
          output: result,
        });
      } else {
  ...
  const handleAnalyzeVideo = async () => {
    ...
    try {
      ...
      const videoResponse = await fetch(
        `/api/youtube/video?url=${encodeURIComponent(videoUrl)}`,
      );
      ...
      if (compUrl) {
        const compResponse = await fetch(
          `/api/youtube/video?url=${encodeURIComponent(compUrl)}`,
        );
        compData = await compResponse.json();
        if (compData.error)
          console.warn("Competitor data fetch error:", compData.error);
      }
  ...
  const handleAnalyzeChannel = async () => {
    ...
    try {
      ...
      // Silent fail or internal error state instead of alert/prompt
      setChannelResult({
        error: "Channel analysis failed. Please check the URL and try again.",
      });
    } finally {
      setIsAnalyzing(false);
    }
  ...
  const handleExport = () => {
    ...
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(channelResult, null, 2));
    const downloadAnchorNode = document.createElement("a");
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute(
      "download",
      `${channelResult.channelName}_report.json`,
    );
          ...
          {(analysisResult || channelResult) && (
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-3xl p-6">
              ...
              <p className="text-sm text-white/80 leading-relaxed">
                {mode === "video"
                  ? analysisResult?.outlierReason
                  : `Based on ${channelResult?.channelName}'s data, the channel is currently seeing a ${channelResult?.averageEngagement} engagement rate. Focus on replicating the "Outlier" videos identified in the list.`}
              </p>
            </div>
          )}
        </div>
          ...
          {isAnalyzing || isGenerating ? (
            ...
            analysisResult ? (
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    {
                      label: "Views",
                      value: analysisResult.realData?.statistics?.viewCount
                        ? Number(
                            analysisResult.realData.statistics.viewCount,
                          ).toLocaleString()
                        : "N/A",
                      icon: Eye,
                      color: "text-blue-400",
                    ...
                    {
                      label: "Likes",
                      value: analysisResult.realData?.statistics?.likeCount
                        ? Number(
                            analysisResult.realData.statistics.likeCount,
                          ).toLocaleString()
                        : "N/A",
                      icon: Heart,
                      color: "text-red-400",
                    ...
                    {
                      label: "Comments",
                      value: analysisResult.realData?.statistics?.commentCount
                        ? Number(
                            analysisResult.realData.statistics.commentCount,
                          ).toLocaleString()
                        : "N/A",
                      icon: MessageCircle,
                    ...
                    {
                      label: "SEO Score",
                      value: analysisResult.seoScore || 85,
                      icon: Search,
                ...
                <div className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                          {analysisResult.hookSuggestions?.map(
                            (hook: string, i: number) => (
                              <div
                                key={i}
                                className="bg-black/40 border border-white/5 p-4 rounded-xl flex items-center justify-between group"
                              >
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                          {analysisResult.contentGaps?.map(
                            (gap: string, i: number) => (
                              <div
                                key={i}
                                className="bg-orange-500/5 border border-orange-500/10 p-4 rounded-xl text-sm text-white/80 flex items-start gap-3"
                              >
                  ...
                  {activeTab === "tags" && (
                    <div className="flex flex-wrap gap-2">
                      {analysisResult.suggestedTags?.map((tag: string) => (
                        <div
                          key={tag}
                          className="bg-orange-500/10 border border-orange-500/30 text-orange-500 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2"
                        >
                  ...
                  {activeTab === "titles" && (
                    <div className="space-y-4">
                      {analysisResult.titleOptimizations?.map(
                        (title: string, i: number) => (
                          <div
                            key={i}
                            className="p-4 bg-black/40 border border-white/5 rounded-xl flex items-center justify-between group"
                          >
                    ...
                    <div className="space-y-4">
                      {!compUrl ? (
                        <div className="text-center py-12 text-white/40">
                          Add a competitor link to see side-by-side comparison.
                        </div>
                      ) : (
                        <div className="overflow-x-auto">
                          <table className="w-full text-left">
                            ...
                            <tbody className="text-sm">
                              {analysisResult.comparisonPoints?.map(
                                (point: any, i: number) => (
                                  <tr
                                    key={i}
                                  ...
                                  >
                                    ...
                                    <td className="py-4 text-white/40 text-xs italic">
                                      {point.insight}
                                    </td>
                                  </tr>
                                ),
              ...
              <div className="h-[600px] border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center text-center p-12">
                ...
                <p className="text-white/40 max-w-md">
                  Enter a video link on the left to see deep analytics.
                </p>
              </div>
            )
          ) : mode === "channel" ? (
            channelResult ? (
                ...
                >
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-8 flex flex-col md:flex-row items-center gap-8">
                    <div className="w-24 h-24 bg-orange-500 rounded-full flex items-center justify-center text-4xl font-bold overflow-hidden">
                      {channelResult.realData?.channel?.snippet?.thumbnails
                        ?.high?.url ? (
                        <img
                          src={
                            channelResult.realData.channel.snippet.thumbnails
                    ...
                    <div className="flex-1 text-center md:text-left">
                      ...
                      <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-white/40">
                        <span className="flex items-center gap-1">
                          ...
                          {channelResult.realData?.channel?.statistics
                            ?.subscriberCount
                            ? Number(
                                channelResult.realData.channel.statistics
                                  .subscriberCount,
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {channelResult.realData?.channel?.statistics
                            ...
                            : channelResult.totalViews}{" "}
                          Total Views
                        </span>
                        <span className="flex items-center gap-1 text-orange-500">
                          <TrendingUp className="w-4 h-4" />{" "}
                          {channelResult.averageEngagement} Engagement
                        </span>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      {
                        label: "Subscribers",
                        value: channelResult.realData?.channel?.statistics
                          ?.subscriberCount
                          ? Number(
                              channelResult.realData.channel.statistics
                                .subscriberCount,
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          {(["all", "outliers", "crushing"] as const).map(
                            (f) => (
                              <button
                                key={f}
                            ...
                            >
                              ...
                              <td className="py-4">
                                {video.isOutlier ? (
                                  ...
                                  <span className="px-2 py-1 rounded text-[10px] font-bold uppercase bg-white/5 text-white/40">
                                    Normal
                                  </span>
                                )}
                              </td>
              ...
              <div className="h-[600px] border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center text-center p-12">
                ...
                <p className="text-white/40 max-w-md">
                  Enter a channel URL to see full performance data and outlier
                  videos.
                </p>
              </div>
            )
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-white capitalize">
                        {studioTool.replace(/([A-Z])/g, " $1").trim()} Assistant
                      </h3>
                  ...
                  {studioResult ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center text-white/20">
                      ...
                      <p className="max-w-xs">
                        Select a tool and enter your topic to generate
                        AI-powered creator assets.
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          ...
          >
            ...
            <div className="text-center mb-8">
              ...
              <h3 className="text-2xl font-bold text-white">Welcome to Insight AI</h3>
              <p className="text-white/40 text-sm mt-2">
                Sign in to access your dashboard and start growing
              </p>
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-white text-gray-900 py-4 rounded-xl font-bold hover:bg-gray-100 transition-all flex items-center justify-center gap-3 shadow-lg"
            >
              {isLoading ? (
...
export default function App() {
  const [view, setView] = useState<
    "landing" | "onboarding" | "checkout" | "dashboard"
  >("landing");
      ...
      <div className="min-h-screen bg-[#050505] text-white selection:bg-orange-500/30">
        ...
        <AnimatePresence mode="wait">
          {view === "landing" ? (
            ...
            >
              <Hero
                onStart={() => (user ? setView("dashboard") : openSignUp())}
              />
                ...
                <div className="grid md:grid-cols-3 gap-12">
                  <div id="outliers" className="space-y-4">
                    ...
                    <p className="text-white/40 leading-relaxed">
                      Our AI scans millions of videos to find "outliers", which
                      are videos that performed ten times better than the
                      channel's average.
                    </p>
                  </div>
                  <div className="space-y-4">
                    ...
                    <p className="text-white/40 leading-relaxed">
                      Analyze competitor performance patterns to identify
                      high-growth opportunities and proven content structures in
                      your niche.
                    </p>
                  </div>
                  <div className="space-y-4">
                    ...
                    <p className="text-white/40 leading-relaxed">
                      Get instant title variations and hook suggestions
                      generated by Gemini AI tailored to your specific niche.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="max-w-7xl mx-auto">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-12">
                    <div className="max-w-xl">
                      ...
                      <p className="text-white/60 mb-8">
                        Access our exclusive library of viral hooks, thumbnail
                        templates, and niche-specific growth playbooks.
                      </p>
                    ...
                    <div className="w-full md:w-80 bg-white/5 border border-white/10 p-8 rounded-3xl">
                      ...
                      <p className="text-xs text-white/40 mb-6">
                        Get weekly outlier reports and AI growth tips delivered
                        to your inbox.
                      </p>
                      {subscribeSuccess ? (
